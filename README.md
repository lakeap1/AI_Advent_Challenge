...
[Задание 2.1 — первый агент](week_2/task_1/README.md): независимый агент с собственным конфигом, входной и выходной политиками, прямым HTTP-вызовом LLM и веб-интерфейсом.

[Задание 2.2 — агент с памятью](week_2/task_2/README.md): диалог в SQLite, восстановление после перезапуска, сохранение токенов, стоимости и результатов политик.

[Задание 2.3 — токены и стоимость диалога](week_2/task_3/README.md): помощник по игровому арту с памятью, оценкой истории, фактическим usage, таблицей расходов и обработкой переполнения контекста.

[Задание 2.4 — сжатие истории](week_2/task_4/README.md): полная история и история с краткой памятью, сравнение сохранения фактов, токенов и стоимости диалога.

[Задание 2.5 — три стратегии контекста](week_2/task_5/README.md): помощник по компьютерной графике и арту со скользящим окном, закреплёнными фактами и независимыми ветками, с фактическим сравнением памяти и расходов.

[Задание 3.1 — явная модель памяти](week_3/task_1/README.md): автоматическое распределение сведений по диалогу, текущей задаче и долговременной памяти, ручное исправление и просмотр источников запроса.

[Задание 3.2 — персонализация](week_3/task_2/README.md): сохранённые профили со стилем, форматом и предпочтениями, которые автоматически учитываются при каждом ответе.

[Задание 3.3 — состояние задачи](week_3/task_3/README.md): этап, текущий шаг и ожидаемое действие сохраняются отдельно от переписки; после паузы и перезапуска можно продолжить с прежнего места.

[Задание 3.4 — инварианты](week_3/task_4/README.md): обязательные правила задачи хранятся отдельно; при обнаруженном конфликте помощник объясняет отказ и сохраняет прежнее состояние.

[Задание 3.5 — контролируемые переходы](week_3/task_5/README.md): выполнение требует утверждённого плана, завершение — проверки результатов; недопустимые переходы блокируются.

[Задание 4.1 — список MCP-инструментов](week_4/task_1/README.md): консольный клиент подключается к публичному Microsoft Learn MCP Server, инициализирует сессию и получает имена инструментов через tools/list.

[Задание 4.2 — источники знаний через MCP](week_4/task_2/README.md): собственный сервер на капсуле обращается к Wikipedia и Stack Exchange; помощник по игровой графике использует найденные материалы в ответе.
