...
function syncControls() {
  ...
  dom.openDialogue.disabled = !ready || !dom.dialogueSelect.value;
  $('#create-dialogue').disabled = !ready || !dom.dialogueName.value.trim();
  $('#create-task').disabled = !ready || !dom.taskName.value.trim();
  $('#send').disabled = !ready || !dom.prompt.value.trim();
  dom.preview.disabled = !ready || !dom.prompt.value.trim();
  $('#save-memory').disabled = !ready || !dom.memoryKey.value.trim() || !dom.memoryValue.value.trim() || !dom.memoryReason.value.trim();
  const editingMemory = Boolean(dom.memoryEditId.value);
  dom.memoryLayer.disabled = editingMemory;
  dom.memoryCategory.disabled = editingMemory;
  dom.memoryScope.disabled = editingMemory;
  dom.memoryKey.readOnly = editingMemory;
  dom.memoryKey.setAttribute('aria-readonly', editingMemory ? 'true' : 'false');
  $('#create-checkpoint').disabled = !ready || !dom.checkpointName.value.trim();
  $('#create-branch').disabled = !ready || !dom.checkpointSelect.value || !dom.branchName.value.trim();
  $('#switch-branch').disabled = !ready || !dom.branchSelect.value;
}

function setBusy(value, label = '') {
  busy = value;
  dom.waiting.hidden = !value;
  dom.agentStatus.textContent = value ? label || 'Выполняется' : ready ? 'Готов к вопросу' : 'Нет соединения';
...
function memoryMoveSelect(entry) {
  ...
  const choices = [
    ...
    ['long_term|profile|user', 'Долговременная · профиль пользователя'],
...
function renderState(state, options = {}) {
  ...
  const nextIdentity = active ? String(active.task_id) + ':' + String(active.id) : null;
  ...
  dom.sessionInfo.textContent = 'Диалог #' + String(activeWorkspace(state)?.id ?? '—') + ' · пользователь ' + String(state.workspace.user_id ?? 'local');
...
function updateMemoryEditorOptions() {
  ...
  if (layer === 'working') {
    ...
    dom.memoryCategory.replaceChildren(
      option('profile', 'Профиль пользователя'),
    ...
    if (['profile', 'decision', 'knowledge'].includes(previousCategory)) dom.memoryCategory.value = previousCategory;
...
document.addEventListener('input', event => {
  if (event.target.matches('#dialogue-name, #task-name, #task-project, #memory-key, #memory-value, #memory-reason, #checkpoint-name, #branch-name')) syncControls();
});
