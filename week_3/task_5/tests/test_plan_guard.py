...
def test_plan_persists_after_restart_and_new_dialogue(tmp_path):
    fake = RawTransport([envelope('stay', 'Композиция', plan=PLAN),
                         envelope('plan_ready', 'Подтверждаю сохранённый план', plan=PLAN), envelope(plan=PLAN)])
    ws = ProfileWorkspace(tmp_path, fake)
    assert ws.agent().run('Композиция').status == 'ok'
    assert ws.agent().run('Подтверждаю сохранённый план').status == 'ok'
...
def test_replanning_requires_new_plan_and_cannot_erase_active_plan(tmp_path):
    fake = RawTransport([envelope('stay', plan=PLAN), envelope('plan_ready', 'Начать', plan=PLAN),
        envelope('stay', plan=[]), envelope('revise_plan', 'Пересмотрим', plan=[]),
        envelope('plan_ready', 'Начать', plan=[]), envelope('stay', plan=PLAN),
        envelope('plan_ready', 'Начать', plan=PLAN)])
    ws = ProfileWorkspace(tmp_path, fake)
    ...
    assert ws.state()['task_state']['stage'] == 'planning'
    assert ws.agent().run('Сохрани новый план').status == 'ok'
    assert ws.agent().run('Начать').status == 'ok'
    ws.close()
