...
def report_for(data, *, passed=False, confirmed=False):
    ...
    report = dict(plan_ready=True, steps=steps, completion_confirmed=confirmed,
                  reason='Проверка по тестовому сценарию')
    if data['requested_event'] == 'plan_ready':
        # This fixture supplies the checker contract only.  The state machine
        # still proves that the plan was saved on an earlier turn.
        report.update(plan_approved=True, approval_evidence=data['current_message'])
    return report


def prepare_saved_plan(store, **changes):
    """Persist a draft with a normal planning ``stay`` before approval."""
    return apply_checked(store, 'stay', **changes)
