...
from state_helpers import report_for, apply_checked, transition
...
class RawTransport:
    ...
    def create(self, payload, timeout):
        ...
        if 'TASK_STAGE_VALIDATION' in payload['instructions']:
            ...
            return response(json.dumps(report_for(data, passed=data['requested_event'] == 'result_reported',
                                                 confirmed=data['requested_event'] == 'finish')))
...
def test_automatic_lifecycle_pause_restart_new_dialogue(tmp_path):
    fake = RawTransport([envelope('plan_ready', 'Керамика'),
    ...
    assert 'roughness 0.2' in fake.calls[-2]['instructions']
...
def test_replan_and_rework_are_automatic_and_notes_persist(tmp_path):
    turns = [('План готов', 'plan_ready', 'execution'),
             ('Изменим цель', 'revise_plan', 'planning'),
             ('Другой план', 'plan_ready', 'execution'),
