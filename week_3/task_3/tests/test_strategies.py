...
def response(text='Ответ', **overrides):
    return dict(status='completed', model=load_config().model, service_tier='default',
...
class Fake:
    ...
    def create(self, payload, timeout):
        ...
        result = self.replies.pop(0) if self.replies else response()
        # Старые сценарии проверяют память/профили, поэтому явный no-op автомата.
        # Ошибки нового протокола покрывает RawTransport без этой адаптации.
        if 'TASK_RESPONSE_JSON' in payload['instructions'] and result.get('status') == 'completed':
            ...
            for item in result.get('output', []):
                for block in item.get('content', []):
                    if block.get('type') == 'output_text':
                        block['text'] = json.dumps(dict(answer=block['text'], event='stay', evidence='',
                            **{key: state[key] for key in ('goal', 'current_step', 'expected_action', 'notes', 'plan')}), ensure_ascii=False)
        return result
