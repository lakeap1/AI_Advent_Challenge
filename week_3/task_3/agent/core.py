...
from .task_state import StateMemoryStore, StageNotReady, task_instructions
from .task_validation import needs_validation, validation_input, parse_validation
from .task_protocol import parse_task_response, response_contract
...
class Agent:
    ...
    def _answer_instructions(self):
        ...
        return instructions + (profile_instructions(profile) if profile else '') + (
            task_instructions(task) + response_contract(task) if task else '')
    ...
    def state(self):
        with self._lock:
            ...
            state['task_state'] = self._task_state()
    ...
    def run(self, prompt, *, use_working=True, use_long_term=True):
        with self._lock:
            ...
            if result.status == 'ok' and meta['context']['task_state'] is not None:
                policy = {**result.output_policy, 'name': 'completed_text_and_task_schema'}
                try:
                    before = meta['context']['task_state']
                    update = parse_task_response(result.text, before, accepted)
                    receipt = None
                    checked = None
                    if needs_validation(before, update):
                        checked, receipt = self._validate_task(before, update, accepted, rid)
                        meta['validation_request_id'] = checked.request_id
                    if checked is not None and checked.status != 'ok':
                        meta['validation_error'] = checked.code
                        result = replace(result, status=checked.status, code='validation_failed',
                            text='Проверка готовности не завершена. ' + checked.text + ' Состояние задачи не изменено; расходы учтены.',
                            output_policy={**policy, 'status': 'rejected'})
                    else:
                        after = self._memory_store.apply_event(self._task_id, update, before,
                                                               prompt=accepted, receipt=receipt)
                        meta['state_update'] = dict(event=update['event'], evidence=update['evidence'],
                                                    before=before, after=after)
                        result = replace(result, text=update['answer'], output_policy=policy)
                except StageNotReady as exc:
                    meta['state_update'] = dict(event='stay', requested_event=update['event'], before=before, after=exc.state)
                    result = replace(result, status='rejected', code='stage_not_ready',
                        text='Переход не выполнен. ' + str(exc) + ' Подтверждённые результаты сохранены.',
                        output_policy={**policy, 'status': 'rejected'})
                except (ValueError, TypeError, RecursionError):
                    result = replace(result, status='rejected', code='invalid_task_response',
                        text='Ответ не прошёл проверку события и полей задачи. Состояние не изменено; расход вызова учтён.',
                        output_policy={**policy, 'status': 'rejected'})
                except StorageError:
