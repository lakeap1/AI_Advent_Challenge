...
function updateTaskStateHelp(taskState) {
  if (taskState?.stage === 'done') {
    ...
    dom.taskStateHelp.textContent = 'Пишите в чат как обычно: состояние и план обновятся вместе со следующим принятым ответом.';
  }
}
...
function renderTaskState(state) {
  ...
  dom.taskProgress.textContent = plan.length
    ...
    : 'План ещё не утверждён автоматически';
  ...
  if (validation) {
    ...
    const verdict = validation.allowed === true ? 'Переход разрешён.' : 'Переход заблокирован.';
...
dom.openDialogue.addEventListener('click', () => {
  ...
  if (switchesTask && !confirmDraftLoss('Открытие другой задачи', {profile: false})) return;
  knownMessageIds = new Set();
