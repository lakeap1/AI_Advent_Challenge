...
from .retrieval import reference_messages
...
def run_guarded(agent, text, use_working, use_long_term, selection=None):
    from .core import AgentResult
    ...
    guard_context = dict(task_state=context['task_state'], messages=list(context_messages))
    if has_rules:
        ...
        if incoming.status != 'ok':
            ...
            agent._store.finish(rid, agent._record(result, meta))
            return result
    retrieved, retrieval_error = agent._retrieve(selection, rid)
    if retrieval_error:
    ...
    if retrieved is not None:
        context_messages[-1:-1] = reference_messages(retrieved['records'])
        context['sent_messages'] = len(context_messages)
        meta['token_metrics'] = agent._counter.measure(text, context_messages[:-1], agent._answer_instructions())
        context['input_text_tokens_estimate'] = meta['token_metrics']['input_text_tokens_estimate']
    ...
    result = replace(agent._invoke(payload, meta, ip, op), request_id=rid, retrieval=retrieved)
    agent._store.pending_result(rid, agent._record(result, meta))
    update = None
    if result.status == 'ok':
        try:
            ...
            result = replace(result, status='rejected', code='invalid_task_response',
                text=('Переход или ответ не принят. ' + str(exc) + ' Состояние и память не изменены; расход вызова учтён.'
                      if isinstance(exc, ValueError) and not isinstance(exc, json.JSONDecodeError)
                      else 'Ответ не прошёл проверку события, плана и полей задачи. Состояние и память не изменены; расход вызова учтён.'),
