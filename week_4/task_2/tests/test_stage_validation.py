...
def validation_json(steps, *, plan_ready=False, confirmed=False, reason='Нужны результаты.',
                    plan_approved=None, approval_evidence=None):
    report = {
    ...
    if plan_approved is not None:
        ...
        report['approval_evidence'] = approval_evidence
...
def test_plan_ready_discards_pending_history_for_the_current_saved_plan():
    state = task_state(stage='planning', results=[step(1)])
    ready_plan = [
    ...
    state['plan'] = ready_plan
    prompt = 'Подтверждаю полный план.'
    text = validation_json([step(1), step(2)], plan_ready=True,
                           reason='План покрывает цель.', plan_approved=True,
                           approval_evidence=prompt)
    report = parse_validation(text, state, update, prompt)
    assert report['steps'] == [step(1), step(2)]
    assert validation_input(state, update, prompt)['previous_results'] == []
...
def test_planning_review_keeps_all_steps_pending_even_when_plan_is_ready():
    ...
    good = validation_json([step(1), step(2), step(3)], plan_ready=True,
                           ...
                           plan_approved=True, approval_evidence='Подтверждаю план.')
    assert parse_validation(good, state, update, 'Подтверждаю план.')['plan_ready'] is True

    bad = validation_json([passed(1, 'Предложено измерение', 'Нужен план.'), step(2), step(3)],
                          plan_ready=True, plan_approved=True, approval_evidence='Подтверждаю план.')
    with pytest.raises(ValueError):
        parse_validation(bad, state, update, 'Подтверждаю план.')
