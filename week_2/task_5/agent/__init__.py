...
def create_agent(config_path=None, api_key=None, db_path=None, mode=None) -> Agent:
    key = os.getenv("OPENAI_API_KEY") if api_key is None else api_key
    config = load_config(config_path)
    if mode is not None: config = replace(config, context_mode=mode)
    return Agent(config, ResponsesTransport(key), SQLiteStore(db_path))
