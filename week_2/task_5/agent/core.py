...
class Agent:
    ...
    def state(self):
        with self._lock:
            ...
            state['requests'].sort(key=lambda r: (r['metadata'].get('parent_request_id', r['id']), 0 if r['metadata'].get('kind') == 'facts' else 1))
    ...
    def preview(self, prompt):
        with self._lock:
            ...
            return dict(status='ok', token_metrics={**self._counter.measure(accepted, context[:-1], self._config.instructions),
                'facts_update_pending': self._config.context_mode == 'facts', 'sent_messages': len(context),
    ...
    def _payload(self, messages, *, facts=False):
        return dict(model=self._config.model,
            instructions=self._config.facts_instructions if facts else self._config.instructions,
    ...
    def run(self, prompt):
        with self._lock:
            ...
            if isinstance(accepted, AgentResult):
                ...
                rid = self._store.begin(self._record(result, meta))
                return replace(result, request_id=rid)
            meta['token_metrics'] = self.preview(accepted)['token_metrics']
            ...
            meta['context'] = dict(sent_message_ids=[m['id'] for m in self._selected(history)],
                ...
                input_text_tokens_estimate=self._counter.history(context) + self._counter.count(self._config.instructions))
