...
class AgentConfig:
    ...
    limits_checked_at: str = '2026-09-13'

    context_mode: str = "compressed"
    keep_last_messages: int = 6
    summary_every_messages: int = 6
    summary_max_tokens: int = 400
    summary_output_tokens: int = 600
    summary_instructions: str = "Сожми историю игрового арта, сохраняя факты, исправления и ограничения."

    def __post_init__(self):
        if self.context_mode not in ("full", "compressed"):
            raise ValueError("Неизвестный режим контекста.")
        for name in ("keep_last_messages", "summary_every_messages", "summary_max_tokens", "summary_output_tokens"):
            if type(getattr(self, name)) is not int or getattr(self, name) <= 0:
                raise ValueError(f"Конфиг: {name} должен быть положительным целым.")
        if not isinstance(self.summary_instructions, str) or not self.summary_instructions.strip():
            raise ValueError("Не заданы инструкции summary.")
        for name in ("model", "instructions", "limits_source", "limits_checked_at"):
