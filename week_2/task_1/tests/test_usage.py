...
def test_provider_counts_and_cost_cache_discount_no_double_reasoning():
    result = Agent(load_config(), FakeTransport(billed_response())).run("Вопрос")
...
def test_incomplete_response_keeps_billable_usage():
    result = Agent(load_config(), FakeTransport(billed_response(status="incomplete"))).run("Вопрос")
...
def test_unknown_pricing_context_does_not_claim_zero(mutation):
    ...
    result = Agent(load_config(), FakeTransport(body)).run("Вопрос")
    assert result.usage.total_tokens == 1200
...
def test_config_rates_change_cost_without_ui_changes():
    ...
    result = Agent(config, FakeTransport(billed_response())).run("Вопрос")
...
def test_usage_survives_http_serialization():
    ...
    client = create_app(Agent(load_config(), FakeTransport(billed_response()))).test_client()
