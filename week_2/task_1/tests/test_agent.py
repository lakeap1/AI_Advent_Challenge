...
def test_input_rejected_without_provider(prompt, code):
    ...
    result = Agent(load_config(), transport).run(prompt)
...
def test_config_controls_request_and_boundary():
    ...
    assert transport.calls == [({"model": "test-model", "instructions": "Custom instruction", "input": "12345", "reasoning": {"effort": config.reasoning_effort}, "max_output_tokens": 99, "store": False, "service_tier": "default"}, 12)]
...
def test_output_policy(output, code):
    result = Agent(load_config(), FakeTransport(output)).run("Вопрос")
...
def test_messages_are_independent():
    transport = FakeTransport()
    agent = Agent(load_config(), transport)
    agent.run("Первый")
    agent.run("Второй")
    assert [call[0]["input"] for call in transport.calls] == ["Первый", "Второй"]
...
def test_transport_errors_are_safe_results(code):
    result = Agent(load_config(), FakeTransport(error=TransportError(code))).run("Вопрос")
...
def test_default_instructions_and_policy_configuration():
    config = load_config()
    assert "без Markdown" in config.instructions
...
def test_toml_is_loaded_and_unknown_keys_rejected(tmp_path):
    ...
    path.write_text(source.read_text(encoding="utf-8").replace("max_input_chars = 4000", "max_input_chars = 3"), encoding="utf-8")
...
def test_lone_surrogate_input_rejected_before_transport():
    transport = FakeTransport()
    result = Agent(load_config(), transport).run('\ud800')
    assert result.code == 'input_invalid'
    assert transport.calls == []

def test_invalid_unicode_output_is_safe_error():
    result = Agent(load_config(), FakeTransport(response('\ud800'))).run('Question')
