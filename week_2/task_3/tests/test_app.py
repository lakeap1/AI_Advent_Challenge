...
def test_page_and_assets():
    ...
    assert 'День 08' in client.get('/').get_data(as_text=True)
    assert client.get('/static/app.js').status_code == 200
    assert client.get('/static/app.css').status_code == 200


def test_preview_endpoint_is_read_only_and_does_not_call_model():
    ...
    before = agent.state()
    result = client.post('/api/preview', json={'prompt':'Текстура сундука'})
    assert result.status_code == 200
    assert result.json['token_metrics']['new_message_tokens_estimate'] > 0
    assert result.headers['Cache-Control'] == 'no-store'
    assert client.post('/api/preview', json={'prompt':''}).status_code == 400
    assert agent.state() == before
    assert transport.calls == []


def test_maximum_unicode_prompt_survives_ascii_json_transport():
    import json
    from agent import load_config
    worker = Stub()
    text = '\U0001f600' * load_config().max_input_chars
    payload = json.dumps({'prompt': text}, ensure_ascii=True)
    response = create_app(worker).test_client().post('/api/ask', data=payload, content_type='application/json')
    assert response.status_code == 200
    assert worker.calls == [text]
